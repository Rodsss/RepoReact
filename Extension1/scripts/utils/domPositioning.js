...
window.myproject1Utils.calculateOptimalMenuPosition = function(
    ...
    if (!chosenPlacement) {
        ...
        if (fallbackLeft + menuWidth > vpRight) { // Too far right
            fallbackLeft = vpRight - menuWidth;
        }
        if (fallbackLeft < vpLeft) { // Too far left
            fallbackLeft = vpLeft;
        }
        if (fallbackTop + menuHeight > vpBottom) { // Too far down
    ...
    return { left: Math.round(finalLeft), top: Math.round(finalTop), placement: chosenPlacement };
};