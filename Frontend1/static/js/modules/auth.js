...
export function initializeAuthFeature(appState, mainRenderCallback) {
  ...
  document
    ...
    .addEventListener("click", handleDelegatedAuthEvents);
  document
    .getElementById("auth-container")
    .addEventListener("submit", handleDelegatedAuthEvents);
}
...
async function handleDelegatedAuthEvents(event) {
  ...
  if (event.type === "click") {
    if (action === "show-register" || action === "show-login") {
      event.preventDefault();
      state.auth.currentView =
        action === "show-register" ? "register" : "login";
