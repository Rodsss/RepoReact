...
export function CollectionsComponent() {
    ...
    const listsHtml = state.collections.lists.length > 0
        ? state.collections.lists.map(list => `<button class="list-item-button" data-action="select-list" data-stack-id="${list.stack_id}">${list.stack_name}</button>`).join('')
        : '<p style="padding: 15px; color: #6a7183;">No lists found. Click the \'+\' icon to create one.</p>';

    const creatorVisibilityClass = state.collections.isCreatorVisible ? 'visible' : '';
    const creatorHtml = `
